...
import { CreateContractBodyType, GetContractListQueryType, RoleName, UpdateContractBodyType } from '@shared/types';
...
export class ContractService {
    ...
    async createContract(clientId: number, body: CreateContractBodyType) {
        try {
            const proposal = await this.contractRepository.findProposalWithJob(body.proposalId);

            if (!proposal) throw ProposalNotFoundException();
            if (proposal.status !== 'PENDING') throw ProposalNotPendingException();
            if (proposal.job.clientId !== clientId) throw ContractForbiddenException();

            const existing = await this.contractRepository.findContractByProposalId(body.proposalId);
    ...
    async updateContract(clientId: number, contractId: number, body: UpdateContractBodyType) {
        try {
            ...
            if (contract.status !== 'PENDING_SIGN') throw ContractNotPendingSignException();
    ...
    async signContract(userId: number, contractId: number) {
        try {
            ...
            if (isFreelancer && contract.signedByFreelancer) throw AlreadySignedException();
            ...
            return await this.contractRepository.signContract(contractId, role, alreadySigned);
        } catch (error) {
            if (error instanceof HttpException) throw error;
            throw FailedToUpdateContractException();
        }
    ...
    async cancelContract(userId: number, contractId: number) {
        try {
            ...
            if (contract.clientId !== userId && contract.freelancerId !== userId) throw ContractForbiddenException();
            if (contract.status === ContractStatus.CANCELLED) throw ContractAlreadyCancelledException();
            if (contract.status === ContractStatus.COMPLETED) throw ContractAlreadyCompletedException();
    ...
    async getContractList(userId: number, roleName: string, query: GetContractListQueryType) {
        try {
    ...
    async getContractDetail(userId: number, roleName: string, contractId: number) {
        try {
            const contract = await this.contractRepository.getContractDetail(contractId);

            if (!contract) throw ContractNotFoundException();

            const isAdmin = roleName === RoleName.ADMIN;
            const isOwner = contract.clientId === userId || contract.freelancerId === userId;
