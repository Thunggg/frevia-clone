...
import { MilestonePaymentStatus, MilestoneStatus, SubmissionStatus } from '@prisma/client';
import { RequestChangesBodyType, RoleName, SubmitMilestoneBodyType } from '@shared/types';
...
export class MilestoneSubmissionService {
    ...
    private async resolveAndGuardContext(
        ...
        const contract = await this.submissionRepository.findContractById(contractId);
        ...
        if (role === 'freelancer' && !isFreelancer) throw SubmissionForbiddenException();
        ...
        const milestone = await this.submissionRepository.findMilestoneById(milestoneId);
        if (!milestone) throw SubmissionMilestoneNotFoundException();
        if (milestone.contractId !== contractId) throw SubmissionForbiddenException();
    ...
    async submitMilestone(
        ...
        try {
            const { milestone } = await this.resolveAndGuardContext(
                userId, roleName, contractId, milestoneId, 'freelancer',
            );
            ...
            return await this.submissionRepository.createSubmission(milestoneId, userId, body);
        } catch (error) {
    ...
    async getSubmissionDetail(
        ...
        try {
            await this.resolveAndGuardContext(
                userId, roleName, contractId, milestoneId, 'participant',
            );
    ...
    async requestChanges(
        ...
        try {
            await this.resolveAndGuardContext(
                userId, roleName, contractId, milestoneId, 'client',
            );
            ...
            return await this.submissionRepository.requestChanges(submissionId, milestoneId, body);
        } catch (error) {
            if (error instanceof HttpException) throw error;
    ...
    async approveMilestone(
        ...
        try {
            ...
            return await this.submissionRepository.approveSubmission(submissionId, milestoneId);
        } catch (error) {
            if (error instanceof HttpException) throw error;
