...
export class MilestoneService {
  ...
  async getMilestoneList(
    ...
    roleName: string,
    contractId: number,
    query: GetMilestoneListQueryType,
  ) {
    try {
      const contract =
  ...
  async getMilestoneDetail(
    ...
    try {
      ...
      const milestone =
        await this.milestoneRepository.getMilestoneDetail(milestoneId);
  ...
  async updateMilestone(
    ...
    try {
      ...
      if (body.amount !== undefined) {
        const totalMilestoneAmount =
          await this.milestoneRepository.getTotalMilestoneAmount(
            contractId,
            milestoneId,
          );
        if (totalMilestoneAmount + body.amount > Number(contract.totalAmount)) {
  ...
  async progressMilestone(
    ...
    try {
      ...
      const inProgressMilestone =
        await this.milestoneRepository.findInProgressMilestone(contractId);

      if (inProgressMilestone && inProgressMilestone.id !== milestoneId) {
        throw MilestoneAlreadyInProgressException();
      }
