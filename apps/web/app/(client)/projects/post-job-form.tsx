...
export function PostJobForm({ open, onOpenChange, onSaved, job }: PostJobFormProps) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto"><DialogHeader><DialogTitle>{job ? "Edit job" : "Post a new job"}</DialogTitle></DialogHeader><form onSubmit={form.handleSubmit(submit)}><FieldGroup>
    ...
    <Controller name="skills" control={form.control} render={({ field, fieldState }) => <Field data-invalid={fieldState.invalid}><FieldLabel>Required skills</FieldLabel><div className="relative"><Input value={skillInput} onChange={(event) => setSkillInput(event.target.value)} placeholder="Search an existing skill..." aria-invalid={fieldState.invalid} />{skillOptions.length > 0 && <div className="absolute z-10 mt-1 w-full rounded-md border bg-popover p-1 shadow-md">{skillOptions.filter((skill) => !field.value.includes(skill.id)).map((skill) => <button key={skill.id} type="button" className="block w-full rounded px-3 py-2 text-left text-sm hover:bg-muted" onClick={() => { field.onChange([...field.value, skill.id]); setSelectedSkills((current) => [...current, skill]); setSkillInput(""); setSkillOptions([]); }}>{skill.name}</button>)}</div>}</div><p className="text-xs text-muted-foreground">Only skills already available in the system can be selected.</p><div className="flex flex-wrap gap-2">{selectedSkills.map((skill) => <Badge key={skill.id}>{skill.name}<button type="button" onClick={() => { field.onChange(field.value.filter((id) => id !== skill.id)); setSelectedSkills((current) => current.filter((item) => item.id !== skill.id)); }}><X className="ml-1 size-3" /></button></Badge>)}</div>{fieldState.invalid && <FieldError errors={[fieldState.error]} />}</Field>} />
